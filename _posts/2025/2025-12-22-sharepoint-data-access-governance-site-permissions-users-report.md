...
Le CSV agrège des métadonnées de site (ID, nom, URL, modèle, admin primaire), des paramètres de partage (invités externes, confidentialité du site), la sensibilité (label appliqué), des volumétries (fichiers), et des compteurs d’items partagés directement ou indirectement avec l’utilisateur. Il inclut aussi les **identifiants Entra ID** et l'UPN de l’utilisateur, plus la date de **. 
...
- **Leaver** : à J+0 exporter le rapport et purger les accès (groupes, liens, partages directs). Conserver l’export dans le dossier d’audit associé au départ de l'utilisateur.
