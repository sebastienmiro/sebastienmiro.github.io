...
Points clés à noter dans cet article :
	•	Le premier Mermaid montre la chaîne d’exclusivité complète (deux Wave WS, production WS, deux Wave Srv, production Srv, sinon catch-all)
	•	Le second Mermaid clarifie les trois types de policies (universelle, catch-all, spécifique)
	•	La stratégie de déploiement Wave1 -> Wave2 -> Production est explicitée
	•	Les anti-patterns incluent maintenant l’oubli des exclusions sur les policies de production et le fait de laisser traîner des postes dans les groupes Wave
Tu valides ce lot 2 ? Si oui, j’enchaîne avec le lot 3 (épisodes 5 et 6 sur l’antivirus et le firewall) en respectant le même modèle.​​​​​​​​​​​​​​​​
...
Le modèle d'exclusivité posé à l'épisode 4 se traduit ici par cinq policies antivirus, toutes autosuffisantes.

| Policy | Cible Intune | Contenu |
|---|---|---|
| MDE-AV-CatchAll | All Devices + Filter Windows-Only + Exclude des 6 groupes | Configuration AV complète et autosuffisante minimale, sans exclusions, pour les machines orphelines |
| MDE-AV-Workstations-Production | Include MDE-Production-Workstations + Exclude Pilot-WS-Wave1 et Wave2 | Configuration AV complète orientée postes : scans planifiés, ajustements CPU, Cloud Block High |
...
| MDE-AV-Servers-Production | Include MDE-Production-Servers + Exclude Pilot-Srv-Wave1 et Wave2 | Configuration AV complète orientée serveurs : Cloud Extended Timeout 50s, CPU faible, scans hors heures |
...
Chaque policy est **autosuffisante**. La policy pilote n'est pas un "delta" qui se superpose à la production : c'est une policy complète qui contient tous les paramètres de la production, plus le durcissement du Cloud Block Level. Cette redondance volontaire évite toute dépendance implicite entre policies.

Les exclusions Intune assurent l'exclusivité. Un poste Wave1 reçoit `MDE-AV-Workstations-Pilot`, point. Il ne reçoit ni la production (parce que celle-ci exclut Wave1) ni le catch-all (parce que celui-ci exclut tous les groupes).

```mermaid
flowchart TD
    A[Poste Windows] --> B{Dans Wave1 ou Wave2 ?}
    B -->|Oui| C[MDE-AV-Workstations-Pilot]
    B -->|Non| D{Dans Production-WS ?<br/>préfixe WRK-}
    D -->|Oui| E[MDE-AV-Workstations-Production]
    D -->|Non| F{Dans Wave1 ou Wave2 Srv ?}
    F -->|Oui| G[MDE-AV-Servers-Pilot]
    F -->|Non| H{Dans Production-Srv ?<br/>préfixe SRV-}
    H -->|Oui| I[MDE-AV-Servers-Production]
    H -->|Non| J[MDE-AV-CatchAll]
